...
class Assets {
	...
	public static $html5                = false;  // Use HTML5 tags
	 ...
	 * @return string
	 */
	public static function css($files = null, $attributes = null, $raw = null)
	{
		self::init();

        if($raw === true || self::$raw) {
            foreach($files as $file) {
                echo self::tag(reduce_double_slashes('/'.self::$assets_dir.'/'.$file), 'css', false, ' rel="stylesheet"');
            }
            return null;
        }
	 ...
	 * @return string
	 */
	public static function css_group($group = null, $files = null, $attributes = null, $raw = null)
	{
		self::$group = $group;
	 ...
	 * @return string
	 */
	public static function js($files = null, $raw = null)
	{
        ...
        if($raw === true || self::$raw) {
            foreach($files as $file) {
                echo self::tag(reduce_double_slashes('/'.self::$assets_dir.'/'.$file), 'js', false, '');
            }
            return null;
        }
	 ...
	 * @return string
	 */
	public static function js_group($group = null, $files = null, $raw = null)
	{
		self::$group = $group;
...
/*					$import_result = self::_process_imports($contents, null, $asset['file']);
	...
	private static function _paths()
	{
		...
		if ( ! self::$base_url) self::$base_url = '/'.self::$assets_dir;  //reduce_double_slashes(config_item('base_url').'/'.self::$assets_dir);
